...
class CategoryFactory(factory.django.DjangoModelFactory):
    class Meta:
        model = Category
        django_get_or_create = ("name", "description", "slug")
...
class PostFactory(factory.django.DjangoModelFactory):
    class Meta:
        model = Post
        django_get_or_create = ("title", "text", "slug", "pub_date")
...
class PostTest(TestCase):
    ...
    def test_create_post(self):
        ...
        self.assertEquals(
            only_post.category.description, "Python the programming language"
        )
        # Check tags
        ...
        self.assertEquals(
            only_post_tag.description, "Pythonsky the programming language"
        )
    ...
    def test_create_romanian_post(self):
        ...
        tag = TagFactory(
            name="răzvansky",
            description="Răzvan the programming language",
            slug="razvansky",
        )
        # Create the post & Set attributes including romanian characters "diacritice"
        post = PostFactory(
            title="Testul cu șțăîâ",
            text="Ăsta este textul de test cu ăîșțâ",
            slug="testul-cu-staia",
        )
        # Add the tag only after creating post
        post.tags.add(tag)
        post.save()
        ...
        self.assertEquals(only_post, post)
        # Check attributes
        self.assertEquals(only_post.title, "Testul cu șțăîâ")
        print("%s" % (only_post.text))
        self.assertEquals(only_post.text, "Ăsta este textul de test cu ăîșțâ")
        ...
        only_post_tag = post_tags[0]
        self.assertEquals(only_post_tag, tag)
        self.assertEquals(only_post_tag.name, "răzvansky")
        self.assertEquals(only_post_tag.description, "Răzvan the programming language")
        self.assertEquals(only_post_tag.slug, "razvansky")
...
class AdminTest(BaseAcceptanceTest):
    ...
    def test_create_category(self):
        with self.settings(
            ...
            response = self.client.post(
                "/administrare/blogengine/category/add/",
                {"name": "python", "description": "The Python programming language"},
                follow=True,
            )
            self.assertEquals(response.status_code, 200)
    ...
    def test_edit_category(self):
        with self.settings(
            ...
            # Edit the category
            response = self.client.post(
                "/administrare/blogengine/category/" + str(category.pk) + "/change/",
                {"name": "perl", "description": "The Perl programming language"},
                follow=True,
            )
            self.assertEquals(response.status_code, 200)
    ...
    def test_delete_category(self):
        with self.settings(
            ...
            # Delete the category
            response = self.client.post(
                "/administrare/blogengine/category/" + str(category.pk) + "/delete/",
                {"post": "yes"},
                follow=True,
            )
            self.assertEquals(response.status_code, 200)
    ...
    def test_create_tag(self):
        with self.settings(
            ...
            # Create the new tag
            response = self.client.post(
                "/administrare/blogengine/tag/add/",
                {
                    "name": "pythonsky",
    ...
    def test_edit_tag(self):
        with self.settings(
            ...
            # Edit the tag
            response = self.client.post(
                "/administrare/blogengine/tag/" + str(tag.pk) + "/change/",
                {"name": "perlsky", "description": "The Perlsky programming language"},
                follow=True,
            )
            self.assertEquals(response.status_code, 200)
            # Check changed successfully
    ...
    def test_delete_tag(self):
        with self.settings(
            ...
            # Delete the tag
            response = self.client.post(
                "/administrare/blogengine/tag/" + str(tag.pk) + "/delete/",
                {"post": "yes"},
                follow=True,
            )
            self.assertEquals(response.status_code, 200)
    ...
    def test_create_admin_post(self):
        with self.settings(
            ...
            response = self.client.get(
                "/administrare/blogengine/post/add/", follow=True
            )
            self.assertEquals(response.status_code, 200)
            # Create the new post
            response = self.client.post(
                "/administrare/blogengine/post/add/",
                {
                    "title": "My first post",
    ...
    def test_edit_post(self):
        with self.settings(
            ...
            # Edit the post
            response = self.client.post(
                "/administrare/blogengine/post/" + str(blogpost.pk) + "/change/",
                {
    ...
    def test_delete_post(self):
        with self.settings(
            ...
            # Delete the post
            response = self.client.post(
                "/administrare/blogengine/post/" + str(post.pk) + "/delete/",
                {"post": "yes"},
                follow=True,
            )
            self.assertEquals(response.status_code, 200)
...
class PostViewTest(BaseAcceptanceTest):
    ...
    def test_index(self):
        ...
        # Create the post
        post = PostFactory(
            title="My first test post for View",
            text="This the first test post for view. And [markdown blog](http://127.0.0.1:8000/)",
            slug="my-first-test-post-for-view",
        )
        post.tags.add(tag)
        # Check post saved
        all_posts = Post.objects.all()
        ...
        # print "%s" %response
        self.assertTrue(
            markdown.markdown(post.text) in response.content.decode("utf-8")
        )
        ...
        self.assertTrue(
            post.pub_date.strftime("%b") in response.content.decode("utf-8")
        )
        ...
        #
        self.assertTrue(
            b'<a href="http://127.0.0.1:8000/">markdown blog</a>' in response.content
        )
    ...
    def test_post_page(self):
        ...
        self.assertTrue(
            b'<a href="http://127.0.0.1:8000/">my first blog post</a>'
            in response.content
        )
    ...
    def test_tag_page(self):
        ...
        # Check empty querySet objects.none() - returns "No posts found"
        self.assertTrue(
            "no coresponding posts found" in response.content.decode("utf-8")
        )
        ...
        # Check the link is marked up properly
        self.assertTrue(
            '<a href="http://127.0.0.1:8000/">my tagged blog post</a>'
            in response.content.decode("utf-8")
        )
...
class PostCreateViewTest(BaseAcceptanceTest):
    ...
    def test_create_post_Auth(self):
        with self.settings(
            ...
            self.assertTrue(b"Site" in response.content)
            # Log out
            self.client.logout()
    ...
    def test_create_valid_post_form(self):
        with self.settings(
            ...
            valid_date_time = timezone.now().strftime(
                "%Y-%m-%d %H:%M:%S"
            )  # make acceptable string
            ...
            # print "%s" %response
            self.assertEquals(response.status_code, 200)
            self.assertTrue(b"Success" in response.content)
            # Check new post now in database
            all_posts = Post.objects.all()
            self.assertEquals(len(all_posts), 1)
